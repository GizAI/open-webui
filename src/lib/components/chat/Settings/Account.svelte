<script lang="ts">
	...
	import { getUserReferral } from '$lib/apis/users';
	...
	onMount(async () => {
		...
		const referralData = await getUserReferral(localStorage.token).catch((error) => {
			console.log(error);
...
<div class="flex flex-col h-full justify-between text-sm">
	<div class=" space-y-3 overflow-y-scroll max-h-[28rem] lg:max-h-full">
						...
						<div class="flex mt-2">
							{#if APIKey}
								<SensitiveInput
									bind:value={APIKey}
									readonly
									class="flex-1 rounded-tl-lg rounded-bl-lg py-2 px-4 text-sm dark:text-gray-300 dark:bg-gray-850 outline-hidden"
								/>

								<button
									class="shrink-0 bg-gray-200 dark:bg-gray-800 hover:bg-gray-300 dark:hover:bg-gray-700 px-3 rounded-tr-lg rounded-br-lg"
									type="button"
									on:click={async () => {
										...
										toast.info($i18n.t('API Key copied to clipboard!'));
									}}
								>
									{#if APIKeyCopied}
										<div>✓</div>
		...
		<div class="pt-2">
			<div class="flex flex-col w-full">
				<div class=" mb-1 text-xs font-medium">{$i18n.t('Referral Code')}</div>
				...
				<div class="flex">
					...
					<button
						...
						type="button"
						on:click={async () => {
							const referralLink = `${WEBUI_BASE_URL}/auth?referrer_code=${referralCode}`;
							...
							toast.info($i18n.t('Referral link copied to clipboard!'));
						}}
					>
