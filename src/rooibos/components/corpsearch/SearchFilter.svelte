	...
	const checkFilter = async (filter: any) => {
		if (filter.id === 'representative_age') {
			...
			if (
				...
				value === '' ||
				(typeof value === 'object' &&
					(Object.keys(value).length === 0 || Object.keys(value).every((key) => value[key] === '')))
			) {
...
{#if group}
	<div>
		<div
			class="bg-white shadow-sm {$mobile
				? 'p-2'
		...
		>
			...
			<div>
					...
					<div class="border rounded-lg p-7 text-gray-900 dark:bg-gray-950 dark:text-gray-200">
						<div class="flex items-center gap-2">
							{#each group.options as option}
								<label
									class="flex items-center hover:bg-gray-100 dark:hover:bg-gray-800 p-1.5 rounded-md transition-all"
								>
									<input
										type="radio"
										name="gender"
				...
				{:else}
					<div
						class="border rounded-lg {shouldShowApplyButton(group.id)
							? 'p-6'
							: 'p-3'} text-gray-900 dark:bg-gray-950 dark:text-gray-200"
					>
						<div
							class="grid {shouldShowApplyButton(group.id)
								? 'grid-cols-1 gap-1'
								: 'grid-cols-2 gap-2'}"
						>
							{#if ['employee_count', 'sales', 'profit', 'net_profit', 'unallocated_profit'].includes(group.id)}
								...
								>
									<label
										class="flex items-center hover:bg-gray-100 dark:hover:bg-gray-800 rounded-md transition-all"
									>
