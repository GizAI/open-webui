		...
		<div class="w-full mb-2.5">
			<div class="flex flex-col w-full px-4 py-2 overflow-y-auto space-y-1">
				<div class="flex items-center justify-between w-full px-0.5 mb-1">
					<h1 class="text-2xl font-semibold mb-1">{companyInfo.company_name}</h1> <!-- mb-1로 간격 축소 -->
				  ...
				  {#if companyInfo.representative}
					<p class="text-sm text-gray-600 flex items-center justify-between">
					  ...
					  <span>
						...
						{#if companyInfo.birth_year} ({companyInfo.birth_year}) {/if}
					  </span>
					</p>
				  {/if}
			  ...
			  <div class="space-y-2">
				...
				{#if companyInfo.industry_major}
				  <p class="text-sm text-gray-600 flex items-center justify-between">
					<span class="flex items-center gap-2">업종</span>
					<span>{companyInfo.industry}</span>
				  </p>
				{/if}
				{#if companyInfo.main_product}
				  <p class="text-sm text-gray-600 flex items-center justify-between">
					<span class="flex items-center gap-2">주요상품</span>
