...
def log_error(request_id, e, error_info=None, additional_info=None):
    ...
    if "'NoneType' object has no attribute 'get'" in str(e):
        print_log("error", f"[{request_id}] NoneType 오류 특별 진단 시작 ---------------")
        ...
        print_log("error", f"[{request_id}] NoneType 오류 특별 진단 종료 ---------------")
    ...
    async def inlet(
        ...
        try:
            ...
            if self.valves.auto_search_mode:
                ...
                if ws_plan is None:
                    print_log("warning", f"[{request_id}] determine_web_search_needed 결과가 None입니다")
                ...
                if ws_result is None:
                    print_log("warning", f"[{request_id}] ws_result 파싱 결과가 None입니다")
            ...
            # 3) Knowledge Base 처리
            print_log("info", f"[{request_id}] 선택된 Knowledge Base 처리 시작, 개수: {len(selected_knowledge_bases)}")
            selected_kb_names = []
                ...
                if kb_id and kb_name:
                    ...
                    if selected_knowledge_base_info:
                        if (
                            ...
                            print_log("warning", f"[{request_id}] selected_knowledge_base_info.data가 없거나 None입니다")
                            continue
            ...
            if selected_kb_names:
                ...
                print_log("info", f"[{request_id}] 선택된 Knowledge Base 이름: {kb_names_str}")
